...
namespace ZenS_Fontend_Test.Service
{
    public class JokeService
    {
        ...
        { 
            User user1 = new User() { FullName = "User1" };
            DataProvider.Ins.DB.Users.Add(user1);
        ...
        public static Joke GetNextJoke(int currentJokeId = 0)
        {
            ...
            var lastJoke = listJoke.LastOrDefault();

            var joke = DataProvider.Ins.DB.Jokes.Where(x => x.Id == currentJokeId + 1).FirstOrDefault();

            return joke == null? AlertEndJoke(): joke;
        }

        public static bool isView(int jokeId)
        {
            if (DataProvider.Ins.DB.ViewedJokes.Where(x=>x.JokeId == jokeId).FirstOrDefault() != null)
            {
                return true;
            }
        ...
        public static bool Vote(int jokeId, int userId, bool like = false)
        {
            ViewedJoke viewedJoke = new ViewedJoke() { JokeId = jokeId, UserId = userId, ViewTime = DateTime.Now, Like = like };

            try
            {
                if (jokeId != -1)
                {
                    //DataProvider.Ins.DB.ViewedJokes.Add(viewedJoke);
                }
